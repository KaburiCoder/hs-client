...
export const useEmitX = <TArgs extends object, TResult>({ ev, onSuccess }: UseEmitArgs<TResult>) => {
  ...
  function emitWithAck(args: TArgs) {
    if (!user?.roomKey || !('eiAuto' in args || patient?.eiAuto)) return;
    return emitAck({ key: user?.roomKey, eiAuto: patient?.eiAuto, ...args })
  }
